function [w, infos] = dp_iht(problem, options)
...
% Modified by H.Kasai on Apr. 17, 2017
    ...
    % store first infos
    clear infos;
    opt_vec = options.opt_vec;
    f_opt = problem.cost(opt_vec);
    infos.iter = iter;
    infos.time = 0;    
    infos.grad_calc_count = 0;    
    f_val = problem.cost(w);
    infos.cost = f_val;     
    optgap = abs(f_val - f_opt);
    infos.optgap = optgap;
    ...
    while (optgap > tol_optgap) && (gnorm > tol_gnorm)  &&(iter < max_iter)        
        ...
        w_old = w;
        scale= 32*max_iter*log(1/delta)/(n^2*epsilon^2);
        dpnoise=normrnd(0, sqrt(scale),[d,1]);
            % update w
        w = w - step * ( grad+dpnoise);   
