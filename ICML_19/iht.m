function [w, infos] = iht(problem, options)
...
% Modified by H.Kasai on Apr. 17, 2017
    ...
    % main loop
    while (optgap > tol_optgap) && (gnorm > tol_gnorm) && (relerror>tol_optgap) &&(iter < max_iter)        
        ...
        gnorm = norm(grad);
        relerror = abs((norm(w)-norm(w_old))/norm(w_old));
        % measure elapsed time
        elapsed_time = toc(start_time);        

        % store infoa
        infos.relerror = [infos.relerror relerror];
    ...
    elseif relerror<tol_optgap
        fprintf('Relative error gap tolerance reached: tol_optgap = %g\n', tol_optgap);
    end    
end
